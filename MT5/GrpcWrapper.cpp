...
namespace TradeField {
    constexpr uint32_t Id = 1;
    constexpr uint32_t BaseId = 2;
    constexpr uint32_t Timestamp = 3;
    constexpr uint32_t Action = 4;
    constexpr uint32_t Quantity = 5;
    constexpr uint32_t Price = 6;
    constexpr uint32_t TotalQuantity = 7;
    constexpr uint32_t ContractNum = 8;
    constexpr uint32_t OrderType = 9;
    constexpr uint32_t MeasurementPips = 10;
    constexpr uint32_t RawMeasurement = 11;
    constexpr uint32_t Instrument = 12;
    constexpr uint32_t AccountName = 13;
    constexpr uint32_t NtBalance = 14;
    constexpr uint32_t NtDailyPnl = 15;
    constexpr uint32_t NtTradeResult = 16;
    constexpr uint32_t NtSessionTrades = 17;
    constexpr uint32_t Mt5Ticket = 18;
    constexpr uint32_t NtPointsPer1kLoss = 19;
    constexpr uint32_t EventType = 20;
    constexpr uint32_t ElasticCurrentProfit = 21;
    constexpr uint32_t ElasticProfitLevel = 22;
    constexpr uint32_t QtTradeId = 23;
    constexpr uint32_t QtPositionId = 24;
    constexpr uint32_t StrategyTag = 25;
    constexpr uint32_t OriginPlatform = 26;
}
...
// Simple RAII socket wrapper to ensure closure on all exit paths
struct SocketHandle {
    SOCKET s;
    explicit SocketHandle(SOCKET sock = INVALID_SOCKET) : s(sock) {}
    SocketHandle(const SocketHandle&) = delete;
    SocketHandle& operator=(const SocketHandle&) = delete;
    SocketHandle(SocketHandle&& other) noexcept : s(other.s) { other.s = INVALID_SOCKET; }
    SocketHandle& operator=(SocketHandle&& other) noexcept {
        if (this != &other) {
            if (s != INVALID_SOCKET) closesocket(s);
            s = other.s; other.s = INVALID_SOCKET;
        }
        return *this;
    }
...
void StreamPollingThread() {
    while (g_streamActive.load()) {
        try {
            // Create persistent connection for streaming
            SocketHandle streamSock(CreateStreamConnection());
            if (streamSock.s == INVALID_SOCKET) {
            ...
            if (!SendGrpcStreamInit(streamSock, "GetTrades")) {
                // RAII will close socket on scope exit
                std::this_thread::sleep_for(std::chrono::seconds(3));
                continue;
            }
            ...
            {
                std::string reqProto = CreateGetTradesRequestProto("hedgebot", 0);
                std::string reqMessage = CreateGrpcMessage(reqProto);
                if (!SendGrpcMessage(streamSock, reqMessage)) {
                    std::this_thread::sleep_for(std::chrono::seconds(3));
                    continue;
                }
            }

            // Main streaming loop - send periodic heartbeats and listen for trades
            auto lastHealthCheck = std::chrono::steady_clock::now();
            char buffer[8192];

            while (g_streamActive.load()) {
                // Send periodic heartbeats to keep stream alive (every 5 seconds)
...
__declspec(dllexport) int __stdcall GrpcReconnect() {
    GrpcShutdown();
    // Reuse the original wide-address to avoid UTF-8 corruption
    return GrpcInitialize(g_serverAddressW.c_str(), g_serverPort);
}
...
__declspec(dllexport) int __stdcall GrpcGetNextTrade(wchar_t* trade_json, int buffer_size) {
    ...
    g_queueCondition.wait(lock, [] { return !g_tradeQueue.empty() || !g_streamActive.load(); });

    if (g_tradeQueue.empty()) {
        trade_json[0] = L'\0';
        return 0; // No trades available (stream inactive or queue empty)
    }

    std::string trade = std::move(g_tradeQueue.front());
    g_tradeQueue.pop();
    lock.unlock();
