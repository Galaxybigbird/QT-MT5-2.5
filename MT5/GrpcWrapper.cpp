#define MT5_GRPC_EXPORTS

#include "cpp-grpc-client/MT5GrpcClient.h"

#include <windows.h>
#include <grpcpp/grpcpp.h>
#include <grpcpp/create_channel.h>
#include <grpcpp/security/credentials.h>
#include "proto/proto/trading.grpc.pb.h"
        ...
        try {
            ...
            while (stream->Read(&trade) && !g_client_state.stop_streaming_) {
                ...
                try {
                    ...
                    std::transform(act.begin(), act.end(), act.begin(), [](unsigned char ch) { return static_cast<char>(std::tolower(ch)); });
...
MT5_GRPC_API int __stdcall GrpcGetNextTrade(wchar_t* trade_json, int buffer_size) {
    try {
        ...
        if (g_client_state.DequeueTrade(trade_str)) {
            if (trade_str.length() >= static_cast<size_t>(buffer_size - 1)) {
...
MT5_GRPC_API int __stdcall GrpcHealthCheck(const wchar_t* request_json, wchar_t* response_json, int buffer_size) {
    try {
        ...
        HealthRequest request;
        request.set_source(request_data.value("source", std::string("MT5_EA")));
        ...
        if (status.ok()) {
            ...
            std::string response_str = response_obj.dump();
            UTF8ToWideString(response_str, response_json, buffer_size);
            g_client_state.is_connected_ = response.status() == "healthy";
            g_client_state.last_health_check_ = std::chrono::steady_clock::now();
            return ERROR_SUCCESS;
        } else {
            g_client_state.SetLastError("Health check request failed: " + status.error_message());
            return ERROR_CONNECTION_FAILED;
        }
...
MT5_GRPC_API int __stdcall GrpcNotifyHedgeClose(const wchar_t* notification_json) {
    try {
        ...
        if (status.ok() && response.status() == "success") {
            ...
            g_client_state.SetLastError("Notify hedge close failed: " + status.error_message());
            return ERROR_CONNECTION_FAILED;
        }
...
MT5_GRPC_API int __stdcall GrpcSubmitElasticUpdate(const wchar_t* update_json) {
    try {
        ...
        if (status.ok() && response.status() == "success") {
            ...
            g_client_state.SetLastError("Submit elastic update failed: " + status.error_message());
            return ERROR_CONNECTION_FAILED;
        }
...
MT5_GRPC_API int __stdcall GrpcSubmitTrailingUpdate(const wchar_t* update_json) {
    try {
        ...
        if (status.ok() && response.status() == "success") {
            ...
            g_client_state.SetLastError("Submit trailing update failed: " + status.error_message());
            return ERROR_CONNECTION_FAILED;
        }
...
MT5_GRPC_API int __stdcall GrpcGetLastErrorMessage(wchar_t* error_message, int buffer_size) {
    try {
        if (!error_message || buffer_size <= 0) {
            return ERROR_INVALID_PARAMS;
        }
        
        std::string error = g_client_state.GetLastError();
        UTF8ToWideString(error, error_message, buffer_size);
        return ERROR_SUCCESS;
    } catch (...) {
        return ERROR_SERIALIZATION;
    }
