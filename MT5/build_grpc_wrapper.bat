...
cl.exe /std:c++17 /EHsc /MD /O2 /DWIN32 /D_WINDOWS /D_USRDLL /DUNICODE /D_UNICODE ^
    /favor:INTEL64 /LD GrpcWrapper.cpp /Fe:MT5GrpcWrapper.dll /link /DEF:MT5GrpcWrapper.def /MACHINE:X64 ^
    ws2_32.lib
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo - gRPC streaming client 
    echo - Trade polling with HTTP/2 over TCP
    echo - Health check functionality
    echo - Trade result submission
    echo - Hedge closure notifications
    echo - Elastic and trailing updates
    echo.
) else (
    echo Build failed!
)
